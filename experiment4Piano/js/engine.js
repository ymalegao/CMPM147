...
function screenToWorld([screen_x, screen_y], [camera_x, camera_y]) {
  let i = (screen_x / tile_width_step_main + screen_y / tile_height_step_main) / 2;
  let j = (screen_y / tile_height_step_main - screen_x / tile_width_step_main) / 2;
  return [Math.floor((i + j) / 2 + camera_x), Math.floor((j - i) / 2 + camera_y)];
}
...
function mouseClicked() {
  ...
  // Display a dad joke for the clicked tile
  

  // Select a random joke
  // console.log(joke); // Print the joke to the console

  // Display the joke for the clicked tile
  
}
function draw() {
  // Keyboard controls!
  background(244);
  //add in the ability to move the camera with the mouse
  // if (mouseIsPressed) {
  //   camera_velocity.x += pmouseX - mouseX;
  //   camera_velocity.y += pmouseY - mouseY;
  // }
  ...
  y0 -= 10; // Adjust this value as needed
  y1 += 10; // Adjust this value as needed
  x0 -= 20; // Adjust this value as needed
  x1 += 20; // 
  let y = 3; // or any other constant value
for (let x = x0; x < x1; x++) {
...
function drawTile([world_x, world_y], [camera_x, camera_y]) {
  ...
  translate(camera_x, camera_y); // Translate to the camera position
  let [screen_x, screen_y] = worldToScreen([world_x, world_y], [0, 0]); // Convert world coordinates to screen coordinates
  translate(screen_x, screen_y); // Translate to the tile position
